 ...
 * Ported from PixelPlayer implementation.
 */
object NeteaseEncryption {
    ...
    private fun aesEncrypt(text: String, key: String, iv: String, mode: String, format: String): String {
        ...
        return when (format.lowercase(Locale.getDefault())) {
            ...
            "hex" -> encrypted.joinToString(separator = "") { "%02x".format(it) }
    ...
    private fun rsaEncrypt(text: String): String {
        return try {
            ...
            val publicKey = KeyFactory.getInstance("RSA").generatePublic(keySpec) as java.security.interfaces.RSAPublicKey

            val message = BigInteger(1, text.toByteArray(StandardCharsets.UTF_8))
            val result = message.modPow(publicKey.publicExponent, publicKey.modulus)
    ...
    fun md5Hex(data: String): String {
        val digest = MessageDigest.getInstance("MD5").digest(data.toByteArray(StandardCharsets.UTF_8))
    ...
    fun weApiEncrypt(payload: Map<String, Any>): Map<String, String> {
        ...
        val encryptedOnce = aesEncrypt(json, PRESET_KEY, IV, "cbc", "base64")
        val params = aesEncrypt(encryptedOnce, secretKey, IV, "cbc", "base64")
    ...
    fun eApiEncrypt(url: String, payload: Map<String, Any>): Map<String, String> {
        ...
        val params = aesEncrypt(message, EAPI_KEY, "", "ecb", "hex").uppercase(Locale.getDefault())
    ...
    private fun toJson(map: Map<String, Any>): String {
        ...
        while (iterator.hasNext()) {
            ...
            builder.append(toJsonValue(entry.value))
            if (iterator.hasNext()) {
                builder.append(',')
            }
        }
    ...
    private fun toJsonValue(value: Any?): String {
        return when (value) {
            ...
            is List<*> -> value.joinToString(prefix = "[", postfix = "]") { toJsonValue(it) }
